...
st.markdown("""
<style>
.main { background-color: #0B0F17; color: #E5E7EB; }
...
model, feature_columns, explainer = load_all()
...
if page == "Overview":
    ...
    st.markdown("""
    Automatically choose the **lowest-cost action** for every transaction using fraud prediction and cost optimization.
    """)
    ...
    if st.session_state.results is None:
        st.info("No analysis yet. Start by uploading data.")
...
# ==============================
    ...
    fraud_cost = col1.slider("Fraud Cost", 1.0, 5.0, 3.0)
    review_cost = col2.slider("Review Cost", 1.0, 20.0, 4.0)
...
# ==============================
# DECISIONS (SIMULATOR + EXPLAINABILITY)
# ==============================
    ...
    if st.session_state.results is None:
        ...
        col1, col2, col3 = st.columns(3)
        col1.metric("Total Cost", f"${sim_df['expected_cost'].sum():,.0f}")
        col2.metric("Automation Rate", f"{(sim_df['optimal_strategy'].str.contains('AI')).mean():.1%}")
        col3.metric("High Risk", f"{(sim_df['risk_probability'] > 0.7).mean():.1%}")
        ...
        st.download_button("Download Results", sim_df.to_csv(index=False), "results.csv")
        ...
        idx = st.selectbox("Select Transaction", sim_df.index)

        row = sim_df.loc[idx]

        st.markdown(f"""
        **Risk Probability:** {row['risk_probability']:.2f}  
        **Decision:** {row['optimal_strategy']}  
        **Expected Cost:** ${row['expected_cost']:.2f}
        """)

        X_row = base_df.loc[[idx], feature_columns]
